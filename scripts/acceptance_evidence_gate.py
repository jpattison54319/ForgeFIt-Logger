...
def audit(run_root: Path, require_contracts: bool) -> dict:
    ...
    complete = bool(
        ...
        and (not require_contracts or evidence_audit.get("contractComplete", False))
    )
    reasons: list[str] = []
    ...
    if require_contracts and not evidence_audit.get("contractComplete", False):
...
def main() -> int:
    ...
    result = audit(args.run.resolve(), args.require_contracts)
