        ...
        ScrollView(showsIndicators: false) {
            VStack(alignment: .leading, spacing: Space.xl) {
                VStack(alignment: .leading, spacing: Space.sm) {
                    ...
                    Text("ForgeFit is built local-first: your training data belongs to you and lives on your device, with optional iCloud sync for your training plan, an optional iCloud Drive backup of your training log, and optional community features you must explicitly opt into.")
                        .font(.system(size: 14)).foregroundStyle(theme.textPrimary)
                }

                section("What we collect",
                        "We operate no servers and collect no personal information. ForgeFit stores your workouts, routines, exercise notes, and settings in a local database on your iPhone. We run no analytics and have no backend. The optional community feature stores only what you explicitly choose to share, in Apple's CloudKit public database — see Community below.")

                section("iCloud sync & backup",
                        "If you are signed into iCloud, ForgeFit syncs your training plan — routines, folders, your exercise library, notes, saved interval and yoga presets, saved insight charts (their definitions only — the numbers they show are recomputed on each device), and your XP progress — across your Apple devices using Apple's CloudKit, stored in your private CloudKit database, encrypted by Apple and accessible only to you.\n\nYour workout history is different: it stays in a local database on each device. To protect it against a lost or replaced phone, ForgeFit writes an optional backup file of your training log to your iCloud Drive, visible in the Files app under ForgeFit. This backup contains only what you logged — sets, reps, weights, durations, effort ratings, notes, cardio splits, and outdoor route maps.\n\nIt never includes heart rate, calories or active energy, step counts, sleep, readiness scores, body weight, daily check-ins, or any other Apple Health data. In line with App Store guidelines, ForgeFit does not store personal health information in iCloud.")
                ...
                section("Community (optional)",
                        "ForgeFit's community features are off until you create a community profile — nothing about you is visible to anyone before that.\n\nIf you opt in, ForgeFit publishes a profile to Apple's CloudKit public database under a handle and display name you choose, along with your XP and level, workout count, lifetime training hours, and lifetime training totals (volume, best estimated 1RM, cardio distance and time, yoga time). You choose who sees it: public (discoverable; anyone can follow) or approve followers (not in discovery; you approve each follower). You can switch visibility at any time.\n\nSharing a workout publishes a training-only copy: exercises, sets, reps, weights, durations, cardio pace and effort, and yoga poses. Shared records have no fields for heart rate, calories, sleep, readiness, body weight, GPS routes, or notes — health and location data cannot appear in shared content by construction. Follows and likes are also stored in the public database. You can unshare any workout at any time, which removes it from the community, and you can delete your entire community profile in the app at any time — see Data deletion.")
                ...
                section("Data deletion",
                        "Deleting the app deletes all local ForgeFit data on that device. Your training plan in iCloud can be removed by deleting routines in the app (deletions sync) or via Settings → Erase All Data. Your training-log backup is an ordinary file you control: delete it in the Files app (iCloud Drive → ForgeFit → Backups), or use Settings → Erase All Data, which also removes the backup. Workouts written to Apple Health remain there under your control and can be deleted in the Health app.\n\nCommunity data is separate: unsharing a workout removes it from the community immediately, and switching your profile to approve-followers takes it out of discovery. To leave the community entirely, use Profile → Community → Delete community profile. This permanently removes your public profile and handle, every workout you shared, and your follows and likes from the public database. Likes and follows that other members created are their records and cannot be deleted from your device; they contain no training or health data and lead nowhere once your profile and workouts are gone. Erase All Data resets only this device — it does not delete your community profile. Use Delete community profile for that.")
